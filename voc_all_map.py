...
from voc_eval_3 import voc_eval
from voc_reval_py3 import do_python_eval
...
def main():
    ...
    # reverse sorted
    # all_weights_files = sorted(all_weights_files, lambda a,b: -1 if int_or_max(file_to_weight_id(a)) > int_or_
    # max(file_to_weight_id(b)) else 1)
    ...
    for weights_file in all_weights_files:
        ...
        cmd = "mv results/comp4* results/{}".format(now)
        print(cmd)
        if os.WEXITSTATUS(os.system(cmd)) != 0:
            assert "{} failed".format(cmd)
